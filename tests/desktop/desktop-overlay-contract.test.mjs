...
test("styles.css scales one content-sized overlay surface for all providers", () => {
  const css = read("src/styles.css");
...
test("Tracked account and provider are persisted to storage and restored on startup", () => {
  ...
  const bootstrap = read("src/hooks/app/useAppSessionBootstrap.ts");
  assert.match(
    constants,
...
test("Overlay refresh button refreshes only the tracked account without triggering full multi-account refresh", () => {
  ...
  assert.match(
    app,
    /refreshTrackedAccountOnly[\s\S]*?fetchAccountUsage\(targetAcc,\s*(?:true|force)\)/,
  );
  assert.match(
    app,
...
test("Overlay moves Claude reset badge to the top-left while other providers keep top-right", () => {
  const overlay = read("src/components/overlay/OverlayApp.tsx");
  const css = read("src/styles.css");
  const app = read("src/App.tsx");
  ...
  assert.match(
    css,
    /\[data-provider="claude"\]\s+\.overlay-reset-badge\s*\{[^}]*left:\s*-4px;[^}]*right:\s*auto;/s,
  );
