...
test("Mono overlay theme follows dark/light app theme with no borders or outlines", () => {
  const styles = fs.readFileSync("src/styles/desktop/overlay-themes.css", "utf8");
  const imports = fs.readFileSync("src/styles.css", "utf8");

  assert.match(imports, /@import ".\/styles\/desktop\/overlay-themes\.css";/);
  assert.match(
    styles,
...
test("glassmorphism stays the default overlay theme", () => {
  ...
  const glass = fs.readFileSync("src/styles/desktop/overlay.css", "utf8");
...
test("overlay theme stylesheet is loaded while UI polish remains the final normalization layer", () => {
  const imports = fs.readFileSync("src/styles.css", "utf8");
  assert.ok(imports.includes('@import "./styles/desktop/overlay-themes.css";'));
  assert.match(imports, /@import "\.\/styles\/desktop\/ui-polish\.css";\s*$/);
});
