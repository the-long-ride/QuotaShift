...
test("Claude guardrail numeric fields stay editable regardless of switch state", () => {
  ...
  const fiveHourInput = inputTagByLabel(
    controls,
    "Claude 5-hour auto-stop threshold percent",
  );
...
test("Claude guardrails are active from either window switch without a master switch", () => {
  ...
  assert.match(hook, /claudePreferences\.fiveHour\.enabled\s*\|\|\s*claudePreferences\.weekly\.enabled/);
...
test("Claude overlay payload carries each enabled guardrail threshold without master gating", () => {
  ...
  assert.doesNotMatch(helper, /preferences\.enabled\s*&&\s*preferences\.(fiveHour|weekly)\.enabled/);
...
test("Claude overlay uses Claude logo avatar and percentage-only outline guardrails", () => {
  ...
  assert.match(styles, /\.overlay-guardrail-badge\s*\{[\s\S]*border-radius:\s*7px/i);
  assert.match(styles, /\.overlay-guardrail-badge\s*\{[\s\S]*border:\s*1px solid rgba\(255,\s*255,\s*255,\s*0\.35\)/i);
...
test("Claude guardrail tooltips describe the configured stop threshold", () => {
  ...
  assert.match(tooltip, /Claude'll be stopped when hit \$\{data\.claudeGuardrails\.fiveHourThresholdPct\}% of 5 hrs limit/);
  assert.match(tooltip, /Claude'll be stopped when hit \$\{data\.claudeGuardrails\.weeklyThresholdPct\}% of weekly limit/);
