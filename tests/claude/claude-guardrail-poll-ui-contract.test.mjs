...
test("tracking Claude with either guardrail on overrides the shared tracked-account runtime poll rate", () => {
  ...
  assert.match(hook, /\[\s*sharedRuntimePollIntervalSecs\s*\]/);
});
...
test("Claude Code guardrails summary displays status and thresholds when collapsed", () => {
  ...
  assert.match(
    controls,
    /!detailsExpanded\s*&&\s*renderCollapsedGuardrailDescription\(preferences\)/,
  );
  assert.doesNotMatch(controls, /Status:\s*ON/);
  assert.doesNotMatch(controls, /Status:\s*OFF/);
  assert.match(
    controls,
    /Poll rate:\s*\$\{poll\}\s*-\s*Suspend eligible Claude Code accounts when usage reaches/,
  );
  ...
  assert.match(
    controls,
    /Only watch running Claude accounts:\s*\$\{preferences\.onlyWatchProcessingAccounts \? "ON" : "OFF"\}/,
  );
  assert.match(
    controls,
    /part === "ON" \|\| part === "OFF" \? <strong key=\{index\}>\{part\}<\/strong>/,
  );
});

test("suspend threshold label, input, and switch share a row with the switch at the right", () => {
  const controls = read("src/components/claude/ClaudeControls.tsx");
  const styles = read("src/styles.css");

  assert.match(
    controls,
    /5-hour suspend %[\s\S]*?aria-label="Claude Code 5-hour auto-suspend threshold percent"[\s\S]*?label="Enable 5-hour Claude Code suspend threshold"/,
  );
  assert.match(
    controls,
    /Weekly suspend %[\s\S]*?aria-label="Claude Code weekly auto-suspend threshold percent"[\s\S]*?label="Enable weekly Claude Code suspend threshold"/,
  );
  assert.match(
    styles,
    /\.claude-threshold-heading\s*\{[^}]*display:\s*grid[^}]*grid-template-columns:\s*minmax\(0,\s*1fr\)\s+42px\s+auto/,
  );
});
...
test("processing-account switch saves the Claude preference and exposes its state accessibly", () => {
  const controls = read("src/components/claude/ClaudeControls.tsx");

  assert.match(controls, /role="switch"/);
  assert.match(controls, /label="Only watch running Claude accounts"/);
  assert.match(controls, /checked=\{preferences\.onlyWatchProcessingAccounts\}/);
  assert.match(
    controls,
    /applyPreferences\(\{\s*\.\.\.preferences,\s*onlyWatchProcessingAccounts\s*\}\)/,
  );
});
