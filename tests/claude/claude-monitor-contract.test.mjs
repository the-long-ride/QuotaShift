...
test("App keeps the existing Claude wiring while useClaudeMonitor enforces split guardrails", () => {
  ...
  const accountHook = read("src/hooks/claude/useClaudeAccountMonitor.ts");
  ...
  assert.match(accountHook, /onlyWatchProcessingAccounts,/);
  ...
  assert.match(hook, /claudePreferences\.onlyWatchProcessingAccounts/);
});
...
test("only-processing preference defaults on and persists independently", () => {
  const storage = createMockStorage();
  const defaults = loadClaudePreferences(storage);
  assert.equal(defaults.onlyWatchProcessingAccounts, true);

  saveClaudePreferences({ ...defaults, onlyWatchProcessingAccounts: false }, storage);
  assert.equal(storage.getItem("quotashift_claude_only_watch_processing_accounts_v1"), "false");
  assert.equal(loadClaudePreferences(storage).onlyWatchProcessingAccounts, false);
});
...
test("Claude preference save reports storage failure and still publishes in-session state", () => {
  ...
  const monitorHook = read("src/hooks/claude/useClaudeMonitor.ts");
...
test("Claude guardrail controls expose independent 5-hour and weekly switches without a master switch", () => {
  ...
  assert.match(controls, /Suspension\s+turns both guardrails off/);
  assert.match(controls, /same process after all triggered limits reset/);
...
test("Claude guardrail controls use the requested left and right column order", () => {
  const controls = read("src/components/claude/ClaudeControls.tsx");
  const styles = read("src/styles.css");

  assert.match(controls, /className="claude-guardrail-grid"/);
  assert.match(controls, /className="claude-guardrail-settings-left"/);
  assert.match(controls, /className="claude-guardrail-settings-right(?:\s|\")/);
  assert.match(controls, /className="claude-auto-resume-group"/);
  assert.match(controls, /className="claude-poll-processing-row"/);
  assert.match(controls, /className="claude-poll-copy"/);
  assert.match(controls, /label="Only watch running Claude accounts"/);
  assert.match(
    controls,
  ...
  assert.ok(
    controls.indexOf('className="claude-processing-watch-group"') <
      controls.indexOf('className="claude-control-field claude-poll-control'),
    "processing-only control must appear before poll rate",
  );
  assert.match(controls, /claude-control-label--active/);
  assert.match(styles, /\.claude-control-label--active/);
  assert.ok(
    controls.indexOf('className="claude-guardrail-settings-left"') <
      controls.indexOf("claude-guardrail-settings-right claude-threshold-grid"),
    "the left settings column must precede the threshold column",
  );
  assert.match(
    styles,
    /\.claude-guardrail-grid\s*\{[\s\S]*display:\s*grid;[\s\S]*grid-template-columns:\s*minmax\(0, 2fr\) minmax\(0, 1fr\);/,
  );
  assert.match(
    styles,
    /\.claude-guardrail-settings-left\s*\{[\s\S]*display:\s*grid;[\s\S]*grid-template-rows:\s*auto auto;/,
  );
  assert.match(
    styles,
    /\.claude-threshold-grid\s*\{[\s\S]*display:\s*grid;[\s\S]*grid-template-rows:\s*repeat\(2, minmax\(0, 1fr\)\);/,
  );
  assert.match(
    styles,
    /\.claude-poll-processing-row\s*\{[\s\S]*grid-template-columns:\s*repeat\(2, minmax\(0, 1fr\)\);/,
  );
  assert.match(
    styles,
    /@media\s*\(max-width:\s*520px\)[\s\S]*\.claude-guardrail-grid\s*\{[\s\S]*grid-template-columns:\s*1fr;/,
  );
});
...
test("collapsed Claude guardrail summary includes auto-resume toggle state", () => {
  ...
  assert.match(controls, /!detailsExpanded && renderCollapsedGuardrailDescription\(preferences\)/);
});
...
test("Claude account cards expose Monitor and wire account-specific tracking to desktop overlay", () => {
  ...
  const usageOverlay = read("src/hooks/app/useAppUsageAndOverlay.ts");
...
test("Claude metric cards use wrapping fit-content flow", () => {
  const styles = read("src/styles/claude/claude.css");
  assert.match(
    styles,
