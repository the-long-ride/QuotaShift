...
const modalParts = modal + read("src/components/codex/CodexPoolModalFooter.tsx");
const member = read("src/components/codex/CodexPoolMemberIdentity.tsx");
...
test("strict discovered validation blocks submit while manual compatibility is warning-only", () => {
  assert.match(modalParts, /validation\.canSave/);
  assert.match(modalParts, /validation\.warning/);
  assert.match(modal, /hasRequiredErrors \|\| !validation\.canSave/);
  assert.match(modalParts, /validation\.reasons/);
});
...
test("member list is a five-row scroller with account identity plan and compatibility", () => {
  ...
  assert.match(
    styles,
    /\.codex-pool-member-list\s*\{[\s\S]*?max-height:\s*170px;[\s\S]*?overflow-y:\s*auto;/,
  );
...
test("member selection uses the approved custom accessible checkbox artwork", () => {
  assert.match(member, /<button[\s\S]*?role="checkbox"/);
  assert.match(member, /aria-checked=\{[^}]+\}/);
  assert.match(
    member,
  ...
  assert.match(member, /m24 24h-24v-24h24\.8v24zm-1\.6-2\.4v-19\.2h-20v19\.2z/);
});
...
test("Codex account card exposes a Show available models icon action before Apply", () => {
  ...
  assert.match(
    tab,
    /Show available models[\s\S]*?(?:Set this account as the active workspace account|card-apply-btn)/,
  );
});
...
test("available models dialog is compact and only its model list scrolls with a thin scrollbar and left-aligned identity", () => {
  assert.match(
    styles,
    /\.codex-model-dialog\s*\{[\s\S]*?height:\s*auto;[\s\S]*?min-height:\s*0;[\s\S]*?max-height:\s*min\(70vh,\s*520px\);/,
  );
  ...
  assert.match(
    styles,
    /\.codex-model-dialog-list::-webkit-scrollbar-track\s*\{[^}]*background:\s*transparent/s,
  );
...
test("global root CSS defines thin 3px scrollbars for all scrollable containers", () => {
  ...
  assert.match(
    styles,
    /::-webkit-scrollbar-thumb\s*,\s*\*::-webkit-scrollbar-thumb\s*\{[^}]*background:\s*var\(--border-hover\)/s,
  );
