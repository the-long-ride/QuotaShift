...
test("Antigravity parser uses the provider name before falling back to the email name", () => {
  const session = {
    "antigravityUnifiedStateSync.oauthToken": "access-token",
    "antigravityUnifiedStateSync.userStatus": JSON.stringify({
      userInfo: { email: "account@example.com" },
    }),
  };

  assert.equal(
    extractAntigravitySessionAccount(session, undefined, { name: "Account Owner" })?.label,
    "Account Owner",
  );
  assert.equal(extractAntigravitySessionAccount(session)?.label, "account");
});
...
test("Codex local session names prefer provider name, then optional fallback, then email name", () => {
  const authForProfile = (profile) => {
    const payload = Buffer.from(JSON.stringify(profile)).toString("base64url");
    return {
      auth_mode: "chatgpt",
      tokens: {
        access_token: "access-token",
        account_id: "acct-label-test",
        id_token: `header.${payload}.signature`,
      },
    };
  };

  const namedAccount = parseCodexLocalAuth(
    authForProfile({ email: "codex@example.com", name: "Provider Name" }),
    "Manual Fallback",
  );
  const fallbackAccount = parseCodexLocalAuth(
    authForProfile({ email: "codex@example.com" }),
    "Manual Fallback",
  );
  const emailAccount = parseCodexLocalAuth(authForProfile({ email: "codex@example.com" }));

  assert.equal(namedAccount?.label, "Provider Name");
  assert.equal(fallbackAccount?.label, "Manual Fallback");
  assert.equal(emailAccount?.label, "codex");
});
