...
const [icon, antigravity, codex] = await Promise.all([
  readFile(
    new URL("../../src/components/common/TrackCurrentAccountIcon.tsx", import.meta.url),
    "utf8",
  ).catch(() => ""),
...
for (const [name, source] of [
  ...
  test(`${name} toolbar exposes the current-account action as icon-only to the left of Best button`, () => {
    ...
    assert.match(source, /Monitor Current Account/);
    ...
    const actionsBlock =
      source.match(/<div className="account-bar-actions">[\s\S]*?<\/div>/)?.[0] || "";
    ...
    assert.ok(
      ...
      "Track current account button must be to the left of the Best button",
    );

    // Verify only icon is rendered inside the button (no visible text)
    assert.match(
      source,
      /<button[^>]*account-action-btn--icon-only[^>]*>\s*<TrackCurrentAccountIcon\s*\/>\s*<\/button>/,
    );
  });
}
