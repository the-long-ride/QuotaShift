...
test("CodexTab uses double-click to monitor and single-click does nothing", () => {
  const code =
    read("src/components/codex/CodexTab.tsx") +
    (fs.existsSync(new URL("../src/components/codex/CodexAccountCard.tsx", import.meta.url))
      ? read("src/components/codex/CodexAccountCard.tsx")
      : "");

  // Single click does nothing
  assert.match(code, /const handleCardClick = \(\) => \{/);
  assert.match(code, /Single click does nothing; double-click marks as monitored/);

  // Double click monitors through onTrack
  ...
  assert.match(
    code,
    /window\.dispatchEvent\(\s*new CustomEvent\("show-tooltip",\s*\{\s*detail:\s*\{\s*target:\s*targetEl,\s*text:\s*"Copied"/,
  );
...
test("AntigravityTab uses double-click to monitor and single-click does nothing", () => {
...
test("App.tsx prompts with CustomDialog confirmation before deleting Antigravity or Codex accounts", () => {
  ...
  assert.doesNotMatch(code, /messageAlign=/);
  assert.match(code, /accountPendingDelete\.email && accountPendingDelete\.name/);
  assert.match(
    code,
    /handleDeleteAntigravityAccount = async \(acc: AntigravityAccount\) => \{[\s\S]*?setAccountPendingDelete/,
  );
  assert.match(
    code,
    /handleDeleteCodexAccount = async \(acc: CodexAccount\) => \{[\s\S]*?setAccountPendingDelete/,
  );

  assert.match(dialogCode, /confirmVariant === "danger"\s*\?\s*"dialog-btn--danger"/);
  assert.doesNotMatch(dialogCode, /messageAlign/);
  assert.match(dialogCode, /className="dialog-message"/);

  assert.match(css, /\.dialog-message\s*\{[^}]*text-align:\s*left;/);
  assert.match(css, /\.dialog-message\s*\{[^}]*justify-content:\s*flex-start;/);
  assert.doesNotMatch(css, /\.dialog-message--left/);
...
test("Light mode uses white background for account cards and slight yellow on hover", () => {
  ...
  assert.match(
    css,
    /\[data-theme="light"\]\s+\.account-card:hover\s*\{[^}]*background:\s*color-mix\(in srgb,\s*var\(--accent-color,\s*#eab308\)\s*8%,\s*transparent\);/s,
  );
  assert.match(
    css,
    /\[data-theme="light"\]\s+\.local-session-card\s*\{[^}]*background:\s*#ffffff;/s,
  );
  assert.match(
    css,
    /\[data-theme="light"\]\s+\.local-session-card:hover\s*\{[^}]*background:\s*color-mix\(in srgb,\s*var\(--accent-color,\s*#eab308\)\s*8%,\s*transparent\);/s,
  );
});
