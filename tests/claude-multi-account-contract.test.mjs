...
test("shared Claude Code guardrails are one-shot and never auto-resume on clear or disable", () => {
  ...
  assert.match(controls, /shared guardrails apply to every discovered Claude Code account/i);
...
test("Claude Code account cards match shared account-card monitoring and two-column quota layout", () => {
  const tab = read("src/components/claude/ClaudeTab.tsx");
  const cards = read("src/components/claude/ClaudeAccountCards.tsx");
  const app = read("src/App.tsx");
  const css = read("src/styles/claude.css") + read("src/styles/claude-accounts.css");
  const flow = read("src/styles/account-card-flow.css");
  ...
  assert.match(
    compact,
    /\[data-card-mode="compact"\] \.claude-account-card \.progress-container[\s\S]*display:\s*block\s*!important/,
  );
});
