...
import { sanitizePollInterval, loadPollIntervalPreference, savePollIntervalPreference } from "./utils/poll-interval"; import { loadAntigravityAccounts, saveAntigravityAccounts, loadCodexAccounts, saveCodexAccounts, loadCodexPools, saveCodexPools, loadCodexModelCache, saveCodexModelCache } from "./utils/app-storage";
import { refreshAntigravityAccountsCloudFirst as refreshAntigravityCloudOps } from "./utils/app-antigravity-ops"; import { fetchCodexUsageData } from "./utils/app-codex-ops"; import { buildClaudeOverlayPayload, buildAntigravityOverlayPayload } from "./utils/app-overlay-helpers"; import { saveAccountOrder, sortByOrder } from "./utils/account-order"; import { buildBackupData, encryptBackup, decryptBackup } from "./utils/app-backup"; import { isNewerVersion } from "./utils/update-policy";
import { Header } from "./components/Header"; import { AntigravityTab } from "./components/AntigravityTab"; import { CodexTab } from "./components/CodexTab"; import { ClaudeTab } from "./components/ClaudeTab"; import { ClaudeLogo } from "./components/ClaudeLogo";
...
export const App: React.FC = () => {
  const [activeTab, setActiveTab] = useState<"antigravity" | "codex" | "claude">("antigravity"), [antigravityAccounts, setAntigravityAccounts] = useState<AntigravityAccount[]>(() => loadAntigravityAccounts()), [activeAntigravityId, setActiveAntigravityId] = useState<string | null>(() => { return localStorage.getItem(ANTIGRAVITY_ACTIVE_ID_KEY); });
  const [codexAccounts, setCodexAccounts] = useState<CodexAccount[]>(() => loadCodexAccounts()), [activeCodexId, setActiveCodexId] = useState<string | null>(() => { return localStorage.getItem(CODEX_ACTIVE_ID_KEY); });
  const [codexPools, setCodexPools] = useState<CodexAccountPool[]>(() => loadCodexPools()), [activeCodexPoolId, setActiveCodexPoolId] = useState<string | null>(() => localStorage.getItem(CODEX_ACTIVE_POOL_ID_KEY)), [codexModelCache, setCodexModelCache] = useState<Record<string, CodexModelCatalogCacheEntry>>(() => loadCodexModelCache());
  const [codexModelScanProgress, setCodexModelScanProgress] = useState({ running: false, total: 0, completed: 0, succeeded: 0, failed: 0 }), [poolRoutingEnabled, setPoolRoutingEnabled] = useState(false), [poolRoutingBusy, setPoolRoutingBusy] = useState(false), [routerStatus, setRouterStatus] = useState<CodexRouterStatus | null>(null);
  const [trackedProvider, setTrackedProvider] = useState<"antigravity" | "codex" | "claude">(() => (localStorage.getItem(OVERLAY_TRACKED_PROVIDER_KEY) as any) || "antigravity"); const [trackedAccountId, setTrackedAccountId] = useState<string | null>(() => localStorage.getItem(OVERLAY_TRACKED_ACCOUNT_ID_KEY)); const [claudeMonitorStatus, setClaudeMonitorStatus] = useState<ClaudeMonitorStatus>({ installed: false, settingsPath: null, source: "none", session: null, localUsage: null, error: null }); const [persistentWorkers, setPersistentWorkers] = useState(false);
  ...
  const persistCodexLastUsed = (id: string, usedAt = Date.now()) => { markAccountLastUsed(codexAccounts, id, usedAt); }; void ((acc: CodexAccount, usedAt: number) => persistCodexLastUsed(acc.id, usedAt));
  const persistAntigravityLastUsed = (id: string, usedAt = Date.now()) => { markAccountLastUsed(antigravityAccounts, id, usedAt); }; const recordRoutedCodexUse = (accountId: string) => persistCodexLastUsed(accountId);
  ...
  const { localAntigravitySession, updateLocalSessionFromStatus, handleLocalAntigravitySessionCaptured, handleAddLocalSessionToMonitored } = useLocalSession(antigravityAccounts, setAntigravityAccounts, refreshAntigravityAccountsCloudFirst);
  const handleTogglePersistentWorkers = async () => { const next = !persistentWorkers; setPersistentWorkers(next); savePersistentWorkerPreference(next); };
  ...
  const handleDeleteAntigravityAccount = async (acc: AntigravityAccount) => {
    ...
    setAccountPendingDelete({ name, email: acc.email || null, onConfirm: async () => {
  ...
  const handleSaveCodexPool = (pool: CodexAccountPool) => { const next = [...codexPools.filter((p) => p.id !== pool.id), { ...pool, activatedAt: Date.now() }]; setCodexPools(next); saveCodexPools(next); }; const handleDeleteCodexPool = (pool: CodexAccountPool) => { const next = codexPools.filter((p) => p.id !== pool.id); setCodexPools(next); saveCodexPools(next); };
  ...
  const handleDeleteCodexAccount = async (acc: CodexAccount) => {
    ...
    setAccountPendingDelete({ name, email: acc.email || null, onConfirm: async () => {
      ...
      if (activeCodexId === acc.id) { const nextAcc = list.length > 0 ? list[0] : null; setActiveCodexId(nextAcc?.id ?? null); if (nextAcc) { localStorage.setItem(CODEX_ACTIVE_ID_KEY, nextAcc.id); await handleApplyCodexAccount(nextAcc); } else localStorage.removeItem(CODEX_ACTIVE_ID_KEY); }
    } });
  };
  ...
  const fetchCodexModelCatalog = async (account: CodexAccount, force = false, isRetry = false): Promise<CodexModelCatalogCacheEntry> => {
    ...
    if (!rawKey.startsWith("{")) {
      const errMsg = "API-key accounts do not expose an account-scoped Codex model catalog", entry: CodexModelCatalogCacheEntry = { accountId: account.id, planName: account.lastPlan ?? null, models: previousEntry?.models ?? [], fetchedAt: previousEntry?.fetchedAt ?? 0, error: errMsg };
  ...
  const fetchAccountUsage = async (account: CodexAccount, force = false): Promise<any> => {
    ...
    try {
      const rawKey = deobfuscate(account.apiKey);
      if (rawKey.startsWith("{")) {
        const oauthData = JSON.parse(rawKey), usageData = await invoke<any>("fetch_chatgpt_usage", { accessToken: oauthData.accessToken, accountId: oauthData.accountId }), limits = usageData.rate_limit || {};
  ...
  const checkForUpdates = async () => {
    try {
      ...
      if (!res.ok) return;
      const releaseData = await res.json(), latestTag = releaseData.tag_name;
  ...
  const triggerRefresh = async (force = false) => {
    ...
    try {
      ...
      checkForUpdates(); const accounts = loadCodexAccounts(); await Promise.all(accounts.map((acc) => fetchAccountUsage(acc, force)));
  ...
  const publishOverlayUpdate = useCallback(() => {
    ...
    if (isClaudeTracked) {
      ...
      payload = { provider: "codex", accountId: acc?.id ?? "codex", label: acc?.label || acc?.email || "Codex", email: acc?.email || "ChatGPT", avatarUrl: acc?.profileUrl ? deobfuscate(acc.profileUrl) : null, tier: cache?.planName?.toUpperCase().includes("PRO") ? "PRO" : "FREE",
  ...
  useEffect(() => {
    let cancelled = false; invoke<ClaudeMonitorStatus>("ensure_claude_statusline_bridge").then((status) => { if (!cancelled) setClaudeMonitorStatus(status); }).catch(() => {});
  ...
  useEffect(() => {
    ...
    if (savedTrackedProvider === "codex") {
      ...
      if (targetAcc) { const initialInfo: CodexMonitoredInfo = { accountId: targetAcc.id, label: targetAcc.label || targetAcc.email || "Codex", primaryPercent: null, primaryLabel: "5h", secondaryPercent: null, secondaryLabel: "wk" }; invoke("set_monitored_codex", { info: initialInfo }).catch(console.warn); }
    ...
    Promise.all(cxAccounts.map((acc) => { setCodexUsageCache((p) => ({ ...p, [acc.id]: { ...p[acc.id], loading: true, isOAuth: deobfuscate(acc.apiKey).startsWith("{") } })); return fetchAccountUsage(acc); })).then(async () => { await maybeAutoFailoverActiveCodexPool(); }).catch(console.error);
    const agAccounts = loadAntigravityAccounts(); agAccounts.forEach((acc) => { setAntigravityUsageCache((p) => ({ ...p, [acc.id]: { ...p[acc.id], loading: true, exactState: "idle", workerMessage: "Refreshing quota summary", error: undefined } })); });
  ...
  useEffect(() => {
    let active = true, unlistenStatus: (() => void) | null = null, unlistenWindow: (() => void) | null = null, unlistenWorker: (() => void) | null = null, unlistenRefreshUsage: (() => void) | null = null, unlistenOverlayVisibility: (() => void) | null = null;
    const setupListeners = async () => {
      ...
      const uWorker = await listen<AntigravityWorkerProgress>("antigravity-worker-progress", (event) => {
        const progress = event.payload, isFinal = ["exact", "cached", "cloud_fallback", "error"].includes(progress.phase);
        setAntigravityUsageCache((p) => ({ ...p, [progress.accountId]: { ...p[progress.accountId], loading: !isFinal, exactState: progress.phase, workerMessage: progress.message } }));
      });
      ...
      const uOverlayVis = await listen<boolean>("overlay-visibility-changed", (_event) => {}); if (!active) uOverlayVis(); else unlistenOverlayVisibility = uOverlayVis;
    };
    setupListeners(); return () => { active = false; unlistenStatus?.(); unlistenWindow?.(); unlistenWorker?.(); unlistenRefreshUsage?.(); unlistenOverlayVisibility?.(); };
  }, []);
  ...
  const handleExportBackup = async () => { setPassMode("export"); setPassOpen(true); }; const handleImportBackup = async (content: string) => { setPendingBackup(content); setPassMode("import"); setPassOpen(true); };
  const handlePassphraseSubmit = async (passphrase: string) => {
    if (passMode === "export") {
      ...
      try {
        ...
        if (Array.isArray(pData.pools)) { const importedIdMap = new Map(); reconcileCodexPools(normalizeCodexPools(pData.pools), codexAccounts); importedIdMap.clear(); }
  ...
  return (
    <div className="app-container">
      ...
      {/* Antigravity Modal */}
      <AddAntigravityAccountModal isOpen={addAgOpen} onClose={() => setAddAgOpen(false)} onAccountAdded={async (id) => { const target = antigravityAccounts.find((a) => a.id === id); if (target) await refreshAntigravityAccountsCloudFirst([target], true); }} loadAccounts={loadAntigravityAccounts} saveAccounts={saveAntigravityAccounts} setActiveAccountId={(id) => { setActiveAntigravityId(id); localStorage.setItem(ANTIGRAVITY_ACTIVE_ID_KEY, id); }} onLocalSessionCaptured={handleLocalAntigravitySessionCaptured} />
