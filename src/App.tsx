...
import type { OverlayAccountData } from "./components/OverlayApp"; import type { ClaudeMonitorStatus, AntigravityAccount, AntigravityUsageCacheEntry, CodexAccount, CodexAccountPool, CodexModelCatalogCacheEntry, CodexRouterStatus } from "./utils/types";
...
import { AddAccountModal } from "./components/AddAccountModal"; import { AddAntigravityAccountModal } from "./components/AddAntigravityAccountModal"; import { PassphraseModal } from "./components/PassphraseModal"; import { CodexPoolModal } from "./components/CodexPoolModal"; import { Toast, ToastKind, ToastMessage } from "./components/Toast"; import { Tooltip } from "./components/Tooltip"; import { useLocalSession } from "./hooks/useLocalSession"; import { useAppThemeAndOverlay } from "./hooks/useAppThemeAndOverlay";
...
export const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const initialPollInterval = loadPollIntervalPreference(); invoke("set_poll_interval", { seconds: BigInt(initialPollInterval) });
    const pollRouterStatus = async () => {
      ...
      if (rst && rst.routedRequestCount > lastSeenRouterRequestCountRef.current) {
    ...
    const id = setInterval(pollRouterStatus, 2000), saved = localStorage.getItem(OVERLAY_TRACKED_PROVIDER_KEY);
    ...
    let unlistenStatus: any, unlistenWindow: any, unlistenRefreshUsage: any, unlistenOverlayVisibility: any;
    const setupListeners = async () => {
      unlistenStatus = await listen("status-updated", (e: any) => {
        setLastFullStatus(e.payload); updateLocalSessionFromStatus(e.payload);
        ...
        const agAccounts = loadAntigravityAccounts(); refreshAntigravityAccountsCloudFirst(agAccounts, true).catch(console.error);
      });
      const uWindow = await listen<boolean>("window-shown", async () => {});
      unlistenWindow = uWindow; unlistenRefreshUsage = await listen("request-refresh-usage", (event: any) => refreshTrackedAccountOnly(event?.payload));
      unlistenOverlayVisibility = await listen<boolean>("overlay-visibility-changed", () => {});
    };
    setupListeners();
    return () => { clearInterval(id); unlistenStatus?.(); unlistenWindow?.(); unlistenRefreshUsage?.(); unlistenOverlayVisibility?.(); };
