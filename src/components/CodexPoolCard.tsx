...
import type { CodexAccount, CodexAccountPool, CodexPoolLaneCapacity, CodexRouterStatus } from "../utils/types";
import { aggregateCodexPoolCapacity } from "../utils/codex-pools";
...
function PoolLane({ name, lane }: { name: string; lane: CodexPoolLaneCapacity }) {
  const pct = lane.capacityPoints > 0
    ? Math.round((lane.remainingPoints / lane.capacityPoints) * 100)
    : 0;
...
export const CodexPoolCard: React.FC<CodexPoolCardProps> = ({
  ...
  const applied = accounts.find((account) => account.id === appliedAccountId && pool.accountIds.includes(account.id));
  const routedAccount = routerStatus?.lastRoutedAccountId
    ? accounts.find((account) => account.id === routerStatus.lastRoutedAccountId && pool.accountIds.includes(account.id))
    : undefined;
  ...
  return (
    <div className={`account-card ${active ? "account-card--active" : ""}`} style={{ marginBottom: "6px" }}>
      <div className="codex-card-header">
        <div style={{ minWidth: 0, flex: 1 }}>
          <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
            <span className="codex-label-text" style={{ fontWeight: 700 }}>{pool.name}</span>
            {active && (
              <span className="card-active-badge"><span className="card-active-dot" />{routedHere ? "Routed" : "Active pool"}</span>
            )}
          </div>
          <div style={{ fontSize: "8.5px", color: "var(--text-secondary)", marginTop: "2px", overflow: "hidden", textOverflow: "ellipsis" }}>
            {pool.model} · {pool.accountIds.length} member{pool.accountIds.length === 1 ? "" : "s"}
            {pool.autoSwitch ? " · Auto-switch" : ""}
          </div>
        </div>
        <div className="codex-card-header-actions" style={{ display: "flex", alignItems: "center", gap: "4px" }}>
          <button className="card-apply-btn" onClick={() => onApply(pool)} disabled={pool.accountIds.length === 0}>
            Apply best
          </button>
          <button className="account-action-btn" onClick={() => onEdit(pool)} data-tooltip="Edit this model pool">Edit</button>
          <button className="codex-card-delete-btn" onClick={() => onDelete(pool)} data-tooltip="Delete this model pool">×</button>
        </div>
      </div>

      <div className="codex-card-info" style={{ display: "flex", justifyContent: "space-between", fontSize: "9px", marginTop: "5px" }}>
      ...
      <div className="quota-limits-container" style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: "8px", marginTop: "10px" }}>
