...
import { invoke } from "@tauri-apps/api/core"; import { emit, listen } from "@tauri-apps/api/event";
import { getCurrentWebviewWindow } from "@tauri-apps/api/webviewWindow"; import { LogicalSize, PhysicalPosition } from "@tauri-apps/api/dpi"; import { currentMonitor, primaryMonitor, availableMonitors } from "@tauri-apps/api/window";
import { detectOverlayHoverZone, getOverlayTooltipText, computeOverlayTooltipPlacement, OverlayHoverZone } from "../../utils/common/overlay-tooltip";
...
const DRAG_THRESHOLD_PX = 4, DOUBLE_CLICK_WINDOW_MS = 500, DOUBLE_CLICK_DISTANCE_PX = 6;
...
export const OverlayApp: React.FC = () => {
  const [data, setData] = useState<OverlayAccountData>(loadInitialOverlayData), [avatarError, setAvatarError] = useState(false);
  const [hoverZone, setHoverZone] = useState<OverlayHoverZone | null>(null), [activeTooltipZone, setActiveTooltipZone] = useState<OverlayHoverZone | null>(null);
  const hoverTimerRef = useRef<number | null>(null), hoverZoneRef = useRef<OverlayHoverZone | null>(null), lastWindowPosRef = useRef<{ x: number; y: number } | null>(null);
  ...
  const monitorBoundsRef = useRef<{ minX: number; maxX: number; minY: number; maxY: number } | null>(null), screenBoundsRef = useRef<{ minX: number; maxX: number; minY: number; maxY: number } | null>(null);
  ...
  const updateMonitorBounds = async () => {
    try {
      ...
      screenBoundsRef.current = { minX: Math.min(...boxes.map((b) => b.pos.x)), maxX: Math.max(...boxes.map((b) => b.pos.x + b.size.width)), minY: Math.min(...boxes.map((b) => b.pos.y)), maxY: Math.max(...boxes.map((b) => b.pos.y + b.size.height)) };
      monitorBoundsRef.current = { minX: screenBoundsRef.current.minX, maxX: screenBoundsRef.current.maxX - winSize.width, minY: screenBoundsRef.current.minY, maxY: screenBoundsRef.current.maxY - winSize.height };
    } catch {}
  };
  ...
  useEffect(() => { const win = getCurrentWebviewWindow(), targetWidth = (data.provider === "codex" || data.provider === "claude") ? 238 : 340; win.setSize(new LogicalSize(targetWidth, 100)).catch(() => {}); }, [data.provider]);
  useEffect(() => { setAvatarError(false); }, [data.avatarUrl]);
  ...
  const clearHover = () => {
    ...
    setHoverZone(null); setActiveTooltipZone(null);
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    if (!pointerDownRef.current && !menuState.isOpen) {
    ...
    if (!dragStartedRef.current) { dragStartedRef.current = true; lastPressRef.current = null; clearHover(); }
  ...
  const handleContextMenu = (e: React.MouseEvent) => {
    clearHover(); e.preventDefault(); e.stopPropagation();
    const winWidth = window.innerWidth || 340, winHeight = window.innerHeight || 100, clickX = e.clientX, clickY = e.clientY, pad = 4;
    const menuWidth = 108; const menuHeight = 58;
    let posX = clickX; if (posX + menuWidth > winWidth - pad) posX = Math.max(pad, clickX - menuWidth);
    let posY = clickY; if (posY + menuHeight > winHeight - pad) posY = Math.max(pad, clickY - menuHeight);
    setMenuState({ isOpen: true, x: Math.round(Math.max(pad, Math.min(posX, winWidth - menuWidth - pad))), y: Math.round(Math.max(pad, Math.min(posY, winHeight - menuHeight - pad))) });
  };
  ...
  const handleOpenDashboard = async (e: React.MouseEvent) => { e.stopPropagation(); setMenuState((prev) => ({ ...prev, isOpen: false })); try { await invoke("show_dashboard"); } catch (err) { console.warn("Failed to open dashboard:", err); } };
  const handleHideOverlay = async (e: React.MouseEvent) => { e.stopPropagation(); setMenuState((prev) => ({ ...prev, isOpen: false })); try { localStorage.setItem("quotashift_overlay_enabled", "false"); await emit("overlay-visibility-changed", false); await invoke("set_overlay_visible", { visible: false }); } catch (err) { console.warn("Failed to hide overlay:", err); } };

  const loading = data.loading ?? false, rows = data.quotaRows ?? [], hasRows = rows.length > 0;
  const clamp = (v: number | null | undefined) => (v !== null && v !== undefined ? Math.max(0, Math.min(100, Math.round(v))) : null);
  const fivePct = clamp(data.fiveHourPercent), weeklyPct = clamp(data.weeklyPercent), initialLetter = (data.label?.trim() || data.email?.trim() || "Q")[0].toUpperCase(), isOpenAI = data.provider === "codex", tierText = resolveTierBadgeText(data.tier);
  const cardClass = `glass-card glass-card--${data.provider}${hasRows ? " glass-card--wide" : ""}`;
  const isDragging = dragStartedRef.current || pointerDownRef.current, showTooltip = activeTooltipZone !== null && !isDragging && !menuState.isOpen;
  ...
  useEffect(() => {
    ...
    if (activeTooltipZone) { setActiveTooltipZone(hoverZone); return; }
    if (!hoverTimerRef.current) {
      hoverTimerRef.current = window.setTimeout(() => { setActiveTooltipZone(hoverZoneRef.current); hoverTimerRef.current = null; }, 500);
    }
  }, [hoverZone, Boolean(activeTooltipZone)]);

  useEffect(() => {
    if (showTooltip && tooltipText) {
      const cardEl = containerRef.current?.querySelector<HTMLElement>(".glass-card") || document.querySelector<HTMLElement>(".glass-card");
      const defaultWidth = (data.provider === "codex" || data.provider === "claude") ? 238 : 340, b = cardEl?.getBoundingClientRect();
      const cardRect = (b && b.width > 0 && b.height > 0) ? { left: b.left, top: b.top, width: b.width, height: b.height } : { left: 4, top: 6, width: defaultWidth - 8, height: 54 };
      const scale = window.devicePixelRatio || 1, screenBounds = screenBoundsRef.current || (typeof window !== "undefined" && window.screen?.availWidth ? { minX: 0, maxX: window.screen.availWidth * scale, minY: 0, maxY: (window.screen.availHeight || 1080) * scale } : null);
      const publish = (wPos: { x: number; y: number }) => { const res = computeOverlayTooltipPlacement({ cardRect, windowPos: wPos, monitorBounds: screenBounds, scale }); emit("overlay-tooltip-data", { text: tooltipText, placement: res.placement, x: res.x, y: res.y, cardCenterX: res.cardCenterX, visible: true }).catch(() => {}); };
      if (lastWindowPosRef.current) publish(lastWindowPosRef.current);
      getCurrentWebviewWindow().outerPosition().then((pos) => { lastWindowPosRef.current = { x: pos.x, y: pos.y }; publish({ x: pos.x, y: pos.y }); }).catch(() => { if (!lastWindowPosRef.current) publish({ x: 0, y: 0 }); });
