...
#[path = "credential_consumer.rs"]
pub mod consumer;
pub use consumer::*;

pub(crate) fn invalidate_cached_credentials() {
...
fn find_antigravity_main_js() -> Option<std::path::PathBuf> {
    #[cfg(target_os = "windows")]
    {
        ...
        let p =
            std::path::PathBuf::from(&local).join("Programs/Antigravity/resources/app/out/main.js");
        if p.exists() {
            return Some(p);
        }
    }
...
fn get_adc_path() -> Option<std::path::PathBuf> {
    #[cfg(target_os = "windows")]
    {
        std::env::var("APPDATA").ok().map(|a| {
            std::path::PathBuf::from(a).join("gcloud/application_default_credentials.json")
        })
    }
...
fn extract_google_client_id(content: &str, prefix: &str) -> Option<String> {
    content
        .split('"')
        .find(|p| p.starts_with(prefix) && p.contains(".apps.googleusercontent.com"))
        .map(|s| s.to_string())
}
...
fn extract_gocsp_secret(content: &str, client_id: &str) -> Option<String> {
    ...
    after
        .split('"')
        .find(|p| p.starts_with("GOCSPX-") && p.len() > 10 && p.len() < 100)
        .map(|s| s.to_string())
}

pub(crate) fn consumer_client_id() -> String {
    if let Some(c) = get_credentials().filter(|c| !c.consumer_client_id.is_empty()) {
        return c.consumer_client_id;
    }
    if let Some((id, _)) = get_ag_consumer_cache()
        .lock()
        .unwrap()
        .as_ref()
...
pub(crate) fn consumer_client_secret() -> String {
    if let Some(c) = get_credentials().filter(|c| !c.consumer_client_secret.is_empty()) {
        return c.consumer_client_secret;
    }
    if let Some((_, secret)) = get_ag_consumer_cache()
        .lock()
        .unwrap()
        .as_ref()
