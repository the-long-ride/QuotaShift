...
fn cloud_code_uses_prod_for_project_discovery_and_daily_for_quota() {
    ...
    assert!(
        ...
        "quota calls must use DAILY Cloud Code"
    );
    assert!(
...
fn oauth_cloud_quota_prefers_authoritative_grouped_summary() {
    ...
    assert!(
        ...
        "saved-account cloud quota must expose Antigravity's grouped quota summary endpoint"
    );
    let summary_call = usage
        .find(".retrieve_user_quota_summary(")
        .expect("saved-account quota must request grouped quota summary first");
    let models_call = usage
        .find(".fetch_available_models(")
        .expect("legacy fetchAvailableModels fallback must remain available");
    assert!(
        summary_call < models_call,
        "grouped quota summary must be attempted before fetchAvailableModels"
    );
...
fn oauth_cloud_quota_does_not_fabricate_weekly_from_retrieve_user_quota() {
    ...
    assert!(
        ...
        "saved-account OAuth cloud quota must not depend on legacy retrieveUserQuota"
    );
    assert!(
